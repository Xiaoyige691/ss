...
class Spider(Spider):
    ...
    def _create_file_item(self, f):
        ...
        if f['ext'] == 'txt':
                    ...
                    if ',#genre#' in content.lower():
                        ...
                        for line in lines[:50]:
                            ...
                            # 检测直接URL（没有逗号分隔）
                                ...
                                # 检查是否是视频流地址
                                if any(ext in line_lower for ext in ['.m3u8', '.m3u', '.flv', '.mp4', '.ts', '.mkv', '.avi']):
                                    url_count += 1
            ...
            # 判断逻辑：如果找到至少一个有效的视频流URL，就当作直播源
            if is_live_source or url_count >= 1:
                ...
                if ',#genre#' in preview.lower():
                    ...
                    for line in lines[:20]:
                        ...
                        # 检测纯URL
                            ...
                            if any(ext in line.lower() for ext in ['.m3u8', '.mp4', '.flv', '.ts', '.mkv', '.avi']):
                                url_count += 1
                            elif re.match(r'https?://[\d.]+:\d+', line):
                                url_count += 1
                        
                        if url_count >= 1 or is_live:
                            break
